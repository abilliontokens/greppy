...
impl WebResourceLoad {
    ...
    /// Continue the real network load, replacing matching request headers.
    /// Unlike [`Self::intercept`], this does not synthesize a response. Servo's
    /// fetch pipeline applies this above TLS, covering HTTPS, redirects, and
    /// subresources.
    pub fn continue_with_headers(mut self, headers: http::HeaderMap) {
        if let Err(error) = self
            .responder
            .send(WebResourceResponseMsg::Continue { headers })
        {
            self.error_sender.raise_response_send_error(error);
        }
    }
